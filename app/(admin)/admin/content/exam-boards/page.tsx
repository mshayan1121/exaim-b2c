...
export default function ExamBoardsPage() {
  const [data, setData] = useState<ExamBoard[]>([])
  ...
  const [selected, setSelected] = useState<ExamBoard | null>(null)
  ...
  async function fetchData() {
    ...
    try {
      ...
      const url = qualificationFilter
        ? `/api/admin/exam-boards?qualification_id=${qualificationFilter}`
  ...
  function openCreate() {
    setFormName("")
    setFormQualificationId(qualifications[0]?.id ?? "")
    setFormError(null)
    setCreateOpen(true)
  }

  function openEdit(eb: ExamBoard) {
    setSelected(eb)
    setFormName(eb.name)
    setFormQualificationId(eb.qualification_id)
    setFormError(null)
    setEditOpen(true)
  }

  function openDelete(eb: ExamBoard) {
    setSelected(eb)
    setFormError(null)
    setDeleteOpen(true)
  }
  ...
  async function handleCreate() {
    ...
    try {
      const res = await fetch("/api/admin/exam-boards", {
        ...
        body: JSON.stringify({
          qualification_id: formQualificationId,
          name: formName,
        }),
      })
      const json = await res.json()
  ...
  async function handleEdit() {
    ...
    setFormError(null)
    try {
      const res = await fetch(`/api/admin/exam-boards/${selected.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          qualification_id: formQualificationId,
  ...
  async function handleDelete() {
    ...
    setFormError(null)
    try {
      const res = await fetch(`/api/admin/exam-boards/${selected.id}`, { method: "DELETE" })
      const json = await res.json()
  ...
  const qualName = (id: string) => qualifications.find((q) => q.id === id)?.name ?? id
    ...
    <div className="p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Select value={qualificationFilter || "all"} onValueChange={(v) => setQualificationFilter(v === "all" ? "" : v)}>
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-surface-raised py-16 dark:border-border dark:bg-surface-raised">
          <Award className="mb-4 h-12 w-12 text-muted-foreground dark:text-muted-foreground" />
          <p className="text-center text-muted-foreground dark:text-muted-foreground">
            No exam boards yet. Create qualifications first, then add exam boards.
          </p>
          <Button onClick={openCreate} className="mt-4 gap-2" disabled={qualifications.length === 0}>
            <Plus className="h-4 w-4" />
            Create exam board
          </Button>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {data.map((eb) => (
                ...
                >
                  <td className="px-4 py-3 text-sm text-foreground dark:text-foreground">{eb.name}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground dark:text-muted-foreground">
                    {qualName(eb.qualification_id)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon-sm" onClick={() => openEdit(eb)}>
                      ...
                      <Button
                        ...
                        onClick={() => openDelete(eb)}
                        disabled={!eb.is_active}
                      >
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create exam board</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Qualification</Label>
              <Select value={formQualificationId} onValueChange={setFormQualificationId}>
                <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={submitting || !formName.trim() || !formQualificationId}>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEdit} disabled={submitting || !formName.trim() || !formQualificationId}>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete exam board</AlertDialogTitle>
            <AlertDialogDescription>
