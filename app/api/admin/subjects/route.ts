...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase
      .from("subjects")
      .select("*, exam_boards(id, name, qualifications(id, name))")
...
export async function POST(request: Request) {
  try {
    ...
    const { exam_board_id, name, display_order } = parsed.data
    const supabase = await createClient()
    const { data, error: insertError } = await supabase
      .from("subjects")
      .insert({ exam_board_id, name, display_order })
      .select()
      .single()
