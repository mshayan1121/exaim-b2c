...
import { generateHierarchy } from "@/lib/claude/hierarchy-generation"
...
export async function POST(request: Request) {
  try {
    ...
    const { qualification, exam_board, subject, variant } = parsed.data
    ...
    return NextResponse.json(
      { success: false, error: "Failed to generate hierarchy. Please try again." },
      { status: 500 }
    )
  }
}
